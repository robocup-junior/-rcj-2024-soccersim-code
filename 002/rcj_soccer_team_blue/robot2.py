...
class MyRobot2(RCJSoccerRobot):
    ...
    def move_until_at_target(self, target_x, target_y):
        ...
        if distance_to_target > 0.2:
            ...
            forward_speed = 9.0  # Forward movement speed
            ...
            if self.is_new_data():
                ...
                if len(set(previous_positions)) == 1:
                    ...
                    if isgoalie == True:
                        ...
                        t_goalie_dpos = 0.6 * r_direction

                        ###if abs(0.6 - robot_y) > 0.02:
                        ###    t_y = 0.6
                        ###    t_x = 0
                        ###elif robot_x <= 0:
                        ###    t_x = -0.4
                        ...
                        if abs(t_goalie_dpos - robot_y) > 0.02:
                            t_y = t_goalie_dpos
                            t_x = 0
                        elif robot_x <= 0:
                            t_x = -0.4
                        ...
                        diff[0] = t_y - robot_y
                        diff[1] = t_x - robot_x
                        ang = math.atan2(diff[0], diff[1])
                        print(diff)
                        print("angle to the target - ", ang)

                        # Get robot's current heading
                        robot_heading = self.get_compass_heading()
                        # angle_to_target = math.atan2(dy, dx)
                        print("robot_heading ", robot_heading)
                        angle_difference = self.normalize_angle(ang - robot_heading)
                        ang = math.degrees(angle_difference)

                        print("angle to the target difference - ", ang)

                        print(diff[0], diff[1])

                        if r_direction == 1:
                            if diff[0] > 0 and diff[1] > 0: ang = ang + 90
                            if diff[0] < 0 and diff[1] < 0: ang = ang + 180
                            if diff[0] > 0 and diff[1] < 0: ang = ang + 90
                        else:
                            if diff[0] > 0 and diff[1] > 0: ang = ang - 90
                            if diff[0] < 0 and diff[1] < 0: ang = ang - 90
                        ...
                        print("angle to the target difference - ", ang)
                        y_goalline_t = 0.6 * r_direction

                        print("abs(t_goalie_dpos - robot_y)", abs(t_goalie_dpos - robot_y))
                        ##if abs(0.6 - robot_y) > 0.02:
                        if abs(t_goalie_dpos - robot_y) > 0.02:
                            if abs(ang) >= 5:
                                left_speed = 5
                            ...
                            if abs(robot_heading - 1.5) < 0.1:
                                print("in here")
                                left_speed = 5
                                right_speed = 5

                        print("check if i can see the ball")

                        if ball_state == BALLSTATE_YESBALL and (robot_y > 0.2 or robot_y < -0.2):
                            # if the goalie can see the ball
                            direction = utils.get_direction(ball_data["direction"])
                            ball_x, ball_y = self.calculate_ball_position(robot_pos, ball_data["direction"],
                                                                          ball_data['strength'])
                            ball_distance = self.get_distance_between_two_positions(robot_x, robot_y, ball_x,
                                                                                    ball_y)
